...
int btti_debugfs_if_prepare_command(u8 cmd_type, struct btti_private *private_data)
{
	int ret = 0;
			   ...
			   struct sk_buff *skb)
{
	...
	if (private_data->card_tx_packet_funcp)
		ret = private_data->card_tx_packet_funcp(private_data, skb);
...
static int btti_hci_if_tx_frame(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	if (private_data->hci_adapter->is_suspending ||
		...
		BT_ERR("[bt sdio hci] %s: Device is suspending or suspended", __func__);
		goto fail;
	}
	...
	default:
		BT_ERR("[bt sdio hci] unknown packet type :%d", hci_skb_pkt_type(skb));
		return -EILSEQ;
	}
...
static int btti_service_work_thread(void *data)
{
	...
	for (;;) {
		...
		if (kthread_should_stop() || private_data->sdio_dev_removed) {
			BT_DBG("[bt sdio hci]work_thread: thread is need to stopped");
			break;
		}
		...
		if (hci_adapter->num_of_interrupt) {
			...
			if (private_data->card_process_rx_funcp) {
				//call to btti_sdio_process_rx
				private_data->card_process_rx_funcp(private_data);
			}
		} else {
			spin_unlock_irqrestore(&private_data->irq_cnt_lock, flags);
		}
		...
		if (private_data->hci_adapter->is_suspended) {
			BT_INFO("[bt sdio hci] work thread not available, is_suspended:%d",
				private_data->hci_adapter->is_suspended);
			continue;
		}
		//handle the TX
...
struct btti_private *btti_hci_add_sdio_dev(void *sdiodev)
{
	...
	private_data->hci_adapter = kzalloc(sizeof(*private_data->hci_adapter),
					    GFP_KERNEL);
	if (!private_data->hci_adapter) {
		BT_ERR("[bt sdio hci] Allocate buffer for btti_hci_adapter failed!");
