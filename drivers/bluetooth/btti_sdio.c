...
static int btti_sdio_if_probe_of(struct device *dev)
{
	if (!dev || !dev->of_node ||
		...
		BT_ERR("[bt sdio] sdio device tree data not available\n");
		return -1;
	}
...
static const struct sdio_device_id btti_sdio_ids[] = {
	{SDIO_DEVICE(SDIO_VENDOR_ID_TI, SDIO_DEVICE_ID_TI_CC33XX),
		.driver_data = (unsigned long)&btti_sdio_cc33xx_bt},
	{ }	/* Terminating entry */
};
					   ...
					   u8 mask)
{
	...
	sdio_writeb(sdiodev->func, mask, sdiodev->reg_map->sdio_enable_int, &ret);
	if (ret) {
...
static void btti_sdio_irq_handler(struct sdio_func *func)
{
	...
	if (!sdiodev || !sdiodev->private_data) {
		BT_ERR("[bt sdio] sdiodev or private_data is NULL, sdiodev=%p", sdiodev);
		return;
	}
	...
	if (ret) {
		BT_ERR("[bt sdio] failed to clear");
		return;
	}
...
static int btti_sdio_rx_packet(struct btti_private *private_data)
{
	...
	if (packet_len <= SDIO_HEADER_LEN || packet_len > HCI_MAX_FRAME_SIZE) {
		BT_ERR("[bt sdio] RX packet length: %d not HCI valid length", packet_len);
		ret = -EINVAL;
		goto exit;
	}
	...
	data_read_size = roundup((packet_len - SDIO_HEADER_LEN), BTSDIO_RX_ALIGN);
	...
	case HCI_EVENT_PKT:
		if (hdev) {
			...
			if (ret < 0)
				BT_ERR("[bt sdio] RX hci_recv_frame failed :%d ", ret);
			ret = 0;
		} else {
			ret = -EPERM;
		}
	...
	default:
		BT_ERR("[bt sdio] RX Unknown packet packet_type:%d", packet_type);
		ret = -ENOENT;
		break;
	}
...
static void btti_acknldg_packet(struct btti_sdio_dev *sdiodev, u8 ack)
{
	...
	sdio_writeb(sdiodev->func, ack, sdiodev->reg_map->sdio_pc_rrt, &ret_ack);
	if (ret_ack && ack == 0)
		BT_ERR("[bt sdio] RX sdio_writesb: send ACK failed: %d", ret_ack);
	if (ret_ack && ack == 1)
		BT_ERR("[bt sdio] RX sdio_writesb: send NACK failed: %d", ret_ack);
}
				       ...
				       struct sk_buff *skb)
{
	...
	case BTTI_BLE_FIRMWARE_UP:
		...
		break;
	default:
		BT_ERR("[bt sdio] unsupported rx vendor event code: %d",
		       vendor_event->event_code);
		ret = -EINVAL;
		break;
	}
...
exit:
	...
	if (ret)
		...
		private_data->btti_dev.hcidev->stat.byte_tx += data_send_size;
	kfree_skb(skb);
	return ret;
}
			      ...
			      const struct sdio_device_id *id)
{
	...
	BT_INFO("[bt sdio] PROBE vendor=0x%x, device=0x%x, class=%d, fn=%d 0x%lx",
...
static int btti_sdio_if_suspend(struct device *dev)
{
	...
	if (func) {
		...
		BT_INFO("[bt sdio] %s: suspend: PM flags = 0x%x", sdio_func_id(func),
			pm_flags);
		if (!(pm_flags & MMC_PM_KEEP_POWER)) {
			BT_ERR("[bt sdio] %s: cannot remain alive while suspended",
			       sdio_func_id(func));
			return -ENOSYS;
		}
		...
		sdiodev = sdio_get_drvdata(func);
		if (!sdiodev || !sdiodev->private_data) {
			BT_ERR("[bt sdio] sdiodev or private_data is not valid");
			return 0;
		}
	} else {
