				       ...
				       unsigned long action, void *data)
{
	...
	if (action & POST_RATE_CHANGE)
		lpi2c_imx->per_clk_rate = clk_get_rate(lpi2c_imx->clks[0].clk);
...
static int lpi2c_imx_probe(struct platform_device *pdev)
{
	...
	ret = clk_notifier_register(lpi2c_imx->clks[0].clk, &lpi2c_imx->clk_change_nb);
	if (ret < 0) {
...
clk_notifier_unregister:
	clk_notifier_unregister(lpi2c_imx->clks[0].clk, &lpi2c_imx->clk_change_nb);
