...
static void sn65dsi83_bridge_enable(struct drm_bridge *bridge)
{
	...
	if (sn65dsi83->panel) {
		DRM_INFO("%s: bus format[0]: %x bpc %d\n", __func__, sn65dsi83->connector.display_info.bus_formats[0], sn65dsi83->connector.display_info.bpc);
...
static int sn65dsi83_bridge_attach(struct drm_bridge *bridge)
{
    ...
    ret = drm_mode_connector_attach_encoder(&sn65dsi83->connector, bridge->encoder);
    if (ret) {
    ...
    const struct i2c_device_id *id)
{
	...
	if (sn65dsi83->vcc1v8)
		regulator_enable(sn65dsi83->vcc1v8);
