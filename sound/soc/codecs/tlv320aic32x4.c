				...
				unsigned int sample_rate)
{
	...
	if (sample_rate <= 48000) {
		...
		dosr_increment = 8;
		aic32x4_set_processing_blocks(codec, 1, 1);
	} else if (sample_rate <= 96000) {
		aosr = 64;
		adc_resource_class = 6;
		dac_resource_class = 8;
		dosr_increment = 4;
		aic32x4_set_processing_blocks(codec, 1, 9);
	} else if (sample_rate == 192000) {
		aosr = 32;
		adc_resource_class = 3;
		dac_resource_class = 4;
		dosr_increment = 2;
		aic32x4_set_processing_blocks(codec, 13, 19);
	} else {
			     ...
			     struct snd_soc_dai *dai)
{
	...
	aic32x4_setup_clocks(codec, params_rate(params));
